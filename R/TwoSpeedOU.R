...
validateModel.2SpOU <- function(tree, model, verbose=FALSE) {
  ...
  if(!all(c('Alpha1','Alpha2', 'Theta', 'Sigma', 'Sigmae')%in%names(model))) {
    stop("model should be a named list with elements 'Alpha1','Alpha2', 'Theta', 'Sigma', 'Sigmae'")
  }
  ...
  if(!is.array(model$Alpha1)|!is.array(model$Alpha2)|!is.array(model$Sigma)|!is.array(model$Sigmae) |
     !all.equal(dim(model$Alpha1[regimes,,,drop=FALSE]),c(R, k, k)) |
     !all.equal(dim(model$Alpha2[regimes,,,drop=FALSE]),c(R, k, k)) |
    ...
    if(verbose) {
      ...
      print('dim Alpha2:')
      print(dim(model$Alpha2))
      print('dim Sigma:')
      print(dim(model$Sigma))
      print('dim Sigmae:')
      print(dim(model$Sigmae))
    }
    stop("Incorrect dimensions for some of the parameters Alpha1, Alpha2, Sigma and Sigmae")
  }

  if(R>1 & (!all.equal(dimnames(model$Alpha1)[[1]], dimnames(model$Theta)[[1]]) |
            !all.equal(dimnames(model$Alpha1)[[1]], dimnames(model$Alpha2)[[1]]) |
            !all.equal(dimnames(model$Alpha1)[[1]], dimnames(model$Sigma)[[1]]) |
            !all.equal(dimnames(model$Alpha1)[[1]], dimnames(model$Sigmae)[[1]]))) {
    if(verbose) {
      print('dimnames Alpha1:')
      print(dimnames(model$Alpha1))
      print('dimnames Alpha2:')
      print(dimnames(model$Alpha2))
    ...
    stop("Disagreeing regime-names (dimnames(X)[[1]]) of Alpha1, Alpha2 Theta, Sigma and Sigmae")
  }
