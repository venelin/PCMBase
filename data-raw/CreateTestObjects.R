...
# Generate some trees, trait data and models for the unit-tests and examples.
# Note that some of this code uses random generation and delegates this generation
# to third-party packages like ape. Hence, it is impossible to guarantee that
# a re-execution of this script would generate an identical PCMBaseTestObjects list.
# Thus, I recommend never executing this script to avoid breaking the unit-tests,
# which currently is enforced by the following if(FALSE) clause.
# If you need to add test-objects to the list, simply add them, without
# recreating the list, paying attention to use different membmer names, and
# overwrite the rda file in the data directory. See the chunk add-to-PCMBaseTestObjects
# in PCMLikTraceExample.Rmd for an example how to do it.
