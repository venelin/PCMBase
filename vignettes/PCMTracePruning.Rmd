...
library(data.table)
library(xtable)
if(!requireNamespace("PCMBaseCpp")) {
  message("Building the vignette requires PCMBaseCpp R-package. Trying to install.")
  status.PCMBaseCpp <- try({
    devtools::install_github("venelin/PCMBaseCpp")
  }, silent = TRUE)
  if(class(status.PCMBaseCpp == "try-error")) {
    stop(
      "The PCMBaseCpp installation did not succeed. The vignette cannot be built.")
  }
}
...
cat(FormatTableAsLatex(
  traceTableCpp[list(pOrder), list(j, i, A_i, b_i, C_i, d_i, E_i, f_i)], 
  type = tableOutputType))
```
