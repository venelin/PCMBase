...
$$
\mathbf{V} = \mathbf{\Sigma}_{x}\mathbf{\Sigma}_{x}^{T}t +  \mathbf{\Sigma}_{e}\mathbf{\Sigma}_{e}^{T}
$$ 
as these compound parameters are calculated at the end of branches.
The package allows for normally distributed $\vec{0}$ mean,
\mathbf{\Sigma}_{e}\mathbf{\Sigma}_{e}^{T} variance of jumps at the end of a branch.
...
Here we just have the $\mathbf{\Sigma}_{x}$ and $\mathbf{\Sigma}_{e}$ matrices
