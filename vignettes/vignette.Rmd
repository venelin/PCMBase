...
One may wonder why in the above description, p = 8 instead of 10 (see also `?PCMNumParams`). The reason is that both, the matrix Sigma and the matrix Sigmae, are symmetric matrices and their matching off-diagonal elements are counted only one time. This is specified in the attribute specParams of the model object (see `?PCMSpecifyParams`). Manipulating the `specParams` attribute of the PCM object is an advanced topic but, quickly, here is how it looks like for the model we've just created:

```{r}
attr(modelBM, "specParams", exact = TRUE)
```

As you can see, the `specParams` attribute of the model is a list with the same named entries as the model itself. Each of these entries defines a scalar, vector or matrix parameter of the model. 

## Model regimes
Model *regimes* are different models associated with different branches of the phylogenetic tree. This is a powerful concept allowing to model different evolutionary modes on different lineages on the tree. Let us create a BM model with two regimes called a and b:

```{r}
modelBM.ab <- PCM("BM", 2, regimes = c("a", "b"))
modelBM.ab
```
...
X0 is defined as a parameter of type "gvector", which stays for a global vector parameter shared by all model regimes. This is also the reason, why the number of parameters is not the double of the number of parameters in the first model:

```{r}
PCMNumParams(modelBM)
PCMNumParams(modelBM.ab)
```
...
modelBM.ab$Sigma[,, "a"] <- rbind(c(1.6, 0),
                                  c(0, 2.4))
modelBM.ab$Sigmae[,, "a"] <- rbind(c(.1, 0),
                                   c(0, .4))
# in regime 'b' there is a correlation between the traits
modelBM.ab$Sigma[,, "b"] <- rbind(c(1.6, .8),
                                  c(.8, 2.4))
modelBM.ab$Sigmae[,, "b"] <- rbind(c(.1, 0),
                                   c(0, .4))
```
...
```{r}
param <- double(PCMNumParams(modelBM.ab))

# load the current model parameters into param
PCMSetOrGetVecParams(modelBM.ab, param, set = FALSE)
...
PCMSetOrGetVecParams(modelBM.ab, param2)
