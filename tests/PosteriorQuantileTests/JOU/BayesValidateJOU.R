...
modelFromVectorJOU <- function(x){
  ...
  Sigmaj.a <- diag(rep(x[9], 2))
  Sigmaj.a[1,2] <- Sigmaj.a[2,1] <- x[10]
  Sigmaj.b <- diag(diag(Sigmaj.a))
...
genParamJOU <- function() {
  ...
  Sigmaj.a <- diag(rep(rexp(1, rate = 10), 2))
  Sigmaj.a[1,2] <- Sigmaj.a[2,1] <- runif(1, min = -Sigmaj.a[1,1] * .9, max = Sigmaj.a[1,1] * .9)
  Sigmaj.b <- diag(diag(Sigmaj.a))
  ...
  c("H2[b11]" = H2.b[1,1], "H2[b22]" = H2.b[2,2], "H2[b12]" = H2.b[1,2],
    ...
    "Sigmaj[a11]" = Sigmaj.a[1,1], "Sigmaj[a12]" = Sigmaj.a[1,2])
}
...
priorJOU <- function(model){
  if(det(model$Sigma[,,1]) < 0 |
    ...
    sum(c(dexp(diag(model$H[,,2]), rate = 1, log = TRUE),
          ...
          dexp(model$Sigmaj[1,1,1], 10, log = TRUE),
          dunif(model$Sigmaj[1,2,1],
                min = -model$Sigmaj[1,1,1] *.9,
                max = model$Sigmaj[1,1,1] * .9, log = TRUE)))
  }
}
...
#   n.batch = c(2, 1, 2, 1, 1, 1, 1, 1),
#   params.batch = expression(H["b,ii"],H["b,ij"],theta["b,i"],Sigma["ab,ii"],Sigma["a,12"],Sigma['e,ab,ii'],Sigma["j,ab,ii"],Sigma["j,a,12"]))
#
...
# set.seed(3)   # good :
# param <- genParamJOU()
# model <- modelFromVectorJOU(param)
#
# mvcond(tree, model, r = 1)$vcov(1, 0)
# mvcond(tree, model, r = 2)$vcov(1, 1)
#
