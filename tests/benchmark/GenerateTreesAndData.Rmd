...
randomOUModel <- function(k) {
  ...
  while(TRUE) {
    ...
    H <- abind::abind(H, along=3, new.names=list(x=NULL, y=NULL, regime=c('a')))
    ...
    cond <- PCMCond(NULL, model)
    eig <- eigen(cond$V(0.01))
...
trees[, tree:=lapply(1:.N, function(i) {
  ...
  tr_len <- POUMM::nodeTimes(tr, tipsOnly = TRUE)
